...
- [ ] **表格行解剖与密度**（大项，值得专门一轮；R7 勘察记录）：列间密度差悬殊（俄语 3 变体 + 注释 vs 孟加拉 1 变体），变体内部 text/rom/tag/note 四层小字堆叠偏碎。**R7 算过：anchor 列宽比不是杠杆**——1.2→1.05 只给每个译文列挪 ~0.6%（1240 壳 ≈ 7px），解决不了密度；真正的工作在变体内部层级（tag/note 的减法与合并、rom/text 字号比、变体间分隔轻量化），需要带截图的排版实验轮。
