...
- 译文格：变体行堆叠，行间虚线。行内两个层级、一个网格：词条层 =（Entry 大字 + 拷贝图标 + 喇叭）同行同级，图标盒高恰为一个词条行框、随流对齐首行；注释层 = rom 与 meta line（tag 仅 `tone=any` 时渲染 tone/gender 标签，与 note 同行同声部）各自横跨整行宽。被筛空的格显示斜纹占位。卡片头复用锚点列三层结构，语言块顺序镜像列序
- 复制：主操作，整行可点——外层行元素转发指针点击，真 `<button>`（只包词条文本，带 `aria-label`）承载语义与键盘，原生激活冒泡回同一处理器，单一路径。affordance 是图标不是文字——文字标签会被误读为独立按钮，且与长词条相撞。复制纯 `text`（不含 rom / tag / note）；成功 = 图标换 accent 对勾 + Toast；失败 fallback `execCommand`
...
- 文字体量补偿：同一名义字号下各文字视觉体量不同，凡多文字同框处经 `--script-scale` 系数取齐灰度（实测依据与系数见 `tokens.css`）；CJK 在短语正文保持 1（阅读字号贴近易读性下限），但作为 Index 标签与斜体拉丁同行时（语言行、锚点槽、语言菜单）经 `.index-native` 类补偿；表格行内锚点词与各列首条词条共享行基线（原生 `vertical-align: baseline`，空格例外）
