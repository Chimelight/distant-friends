...
**维护规则**：正文写当前事实，不打日期补丁，同一件事只在一处记录。日期只出现在三处：§9 决策日志、§10 迭代账本、附录 C 版本史。推翻旧决策时，在 §9 末尾追加一两行，不改写旧条目；完成一轮打磨，在 §10 记一行。阅读时不必通读全文：改内容读 §3 与 README；做打磨轮读 §10、§9 末尾数条及相关的 §4 小节；性能、无障碍、部署、CI 均在 §7。
...
- **v4.3**（2026-07-12）— 开篇三节改第一人称，措辞规范化：口语表达收敛为书面语，语义不变

*文档版本 v4.3 · 2026-07-12*
