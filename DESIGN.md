...
- [ ] **换列/加删列的连续性动画**（桌面炫技主菜）：列切换目前瞬时重排；用 FLIP（或 View Transitions）让列头与整列内容平滑滑入/让位/淡出，anchor 切换时金色列底跟着移动。数据不动、纯视觉连续性。
- [ ] **收藏的仪式感**：星标动画——描边→填充 + 一圈墨点/金屑绽放（CSS-only，reduced-motion 降级为直接填充）；"Starred only" 空态信笺化文案；场景标题旁星数微标；星标行在表格里一枚极轻的金角记号。
