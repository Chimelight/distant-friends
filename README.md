...
4. **Every variant must earn its seat.** Most phrases have countless casual paraphrases; listing several synonyms reads as noise, not coverage. A second variant exists only for a grammatical split (gender, politeness), a genuinely different function, or a fixed expression every native reaches for. One canonical form beats a sample of alternatives.
